...
from volkit import price_euro_future, estimate_vol_from_option_prices
...
def test_price_and_iv_roundtrip_calls_and_puts():
    ...
    iv_call = estimate_vol_from_option_prices(C_call, F, K, T, r, "call")
    iv_put = estimate_vol_from_option_prices(C_put, F, K, T, r, "put")
...
def test_iv_at_cap_returns_inf():
    ...
    iv = estimate_vol_from_option_prices(C_cap, F, K, T, r, "call")
    assert np.isinf(iv)
...
def test_iv_t0_returns_zero_when_intrinsic():
    ...
    iv = estimate_vol_from_option_prices(C_intr, F, K, T, r, "call")
    assert iv == 0.0
...
def test_iv_below_noarb_is_nan():
    ...
    iv = estimate_vol_from_option_prices(C, F, K, T, r, "call")
    assert np.isnan(iv)
...
def test_cp_vector_broadcast_in_iv():
    ...
    iv = estimate_vol_from_option_prices(C, F, K, T, r, ["call", "put"])
...
def test_iv_bisection_early_tight_break():
    ...
    iv = estimate_vol_from_option_prices(
...
def test_iv_returns_out_when_all_fail_after_growth():
    ...
    C = price_euro_future(F, K, T, r, sigma_true, "call")
    iv = estimate_vol_from_option_prices(C, F, K, T, r, "call", sigma_max=0.3)
    assert np.isnan(iv)
