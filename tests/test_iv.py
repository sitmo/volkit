...
from volkit import price_euro_future, implied_vol_euro_future
...
def test_price_and_iv_roundtrip_calls_and_puts():
    ...
    iv_call = implied_vol_euro_future(C_call, F, K, T, r, "call")
    iv_put = implied_vol_euro_future(C_put, F, K, T, r, "put")
...
def test_iv_at_cap_returns_inf():
    ...
    iv = implied_vol_euro_future(C_cap, F, K, T, r, "call")
    assert np.isinf(iv)
...
def test_iv_t0_returns_zero_when_intrinsic():
    ...
    iv = implied_vol_euro_future(C_intr, F, K, T, r, "call")
    assert iv == 0.0
...
def test_iv_below_noarb_is_nan():
    ...
    iv = implied_vol_euro_future(C, F, K, T, r, "call")
    assert np.isnan(iv)
...
def test_cp_vector_broadcast_in_iv():
    ...
    iv = implied_vol_euro_future(C, F, K, T, r, ["call", "put"])
...
def test_iv_returns_out_when_all_fail_after_growth():
    ...
    C = price_euro_future(F, K, T, r, sigma_true, "call")
    iv = implied_vol_euro_future(C, F, K, T, r, "call", sigma_max=0.3)
    assert np.isnan(iv)
