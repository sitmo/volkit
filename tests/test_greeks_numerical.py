...
def n_delta(F, K, T, r, sigma, cp, h):
    return (_price(F+h, K, T, r, sigma, cp) - _price(F-h, K, T, r, sigma, cp)) / (2*h)

def n_gamma(F, K, T, r, sigma, cp, h):
    return (_price(F+h, K, T, r, sigma, cp) - 2*_price(F, K, T, r, sigma, cp) + _price(F-h, K, T, r, sigma, cp)) / (h*h)

def n_vega(F, K, T, r, sigma, cp, k):
    return (_price(F, K, T, r, sigma+k, cp) - _price(F, K, T, r, sigma-k, cp)) / (2*k)

def n_theta_calendar(F, K, T, r, sigma, cp, t):
    # calendar theta: dV/dt = - dV/dT
    t = min(t, 0.5*T)
    return -(_price(F, K, T+t, r, sigma, cp) - _price(F, K, T-t, r, sigma, cp)) / (2*t)

def n_rho(F, K, T, r, sigma, cp, q):
    return (_price(F, K, T, r+q, sigma, cp) - _price(F, K, T, r-q, sigma, cp)) / (2*q)

def n_dual_delta(F, K, T, r, sigma, cp, k):
    return (_price(F, K+k, T, r, sigma, cp) - _price(F, K-k, T, r, sigma, cp)) / (2*k)

def n_vanna(F, K, T, r, sigma, cp, h, k):
    return (
        _price(F+h, K, T, r, sigma+k, cp) - _price(F+h, K, T, r, sigma-k, cp)
        - _price(F-h, K, T, r, sigma+k, cp) + _price(F-h, K, T, r, sigma-k, cp)
    ) / (4*h*k)

def n_vomma(F, K, T, r, sigma, cp, k):
    return (_price(F, K, T, r, sigma+k, cp) - 2*_price(F, K, T, r, sigma, cp) + _price(F, K, T, r, sigma-k, cp)) / (k*k)

def n_lambda(F, K, T, r, sigma, cp, h):
    V  = _price(F, K, T, r, sigma, cp)
