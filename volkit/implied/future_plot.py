    ...
    *,
    ...
    if res is None:
        if ax is None:
            ...
            ax.set_xlabel("Strike K"); ax.set_ylabel("Implied forward F")
    ...
    finite = np.isfinite(K) & np.isfinite(L) & np.isfinite(U) & np.isfinite(D_star) & (D_star > eps)
    Fi_lo = np.empty_like(K); Fi_hi = np.empty_like(K)
    ...
    if not ok.any():
        if ax is None:
            fig, ax = plt.subplots(1, 1, figsize=(10, 5.5))
            ax.text(0.5, 0.5, "No finite forward intervals at D*.", ha="center", va="center", transform=ax.transAxes)
            ax.set_xlabel("Strike K"); ax.set_ylabel("Implied forward F")
    ...
    ax.set_title(f"F_bid={res.F_bid:.6g}, F_ask={res.F_ask:.6g}, D_bid={res.D_min:.6g}, D_ask={res.D_max:.6g}")
    ...
    green_proxy = mlines.Line2D([], [], color="green", linewidth=3.0, label=f"Kept ({n_keep})")
    red_proxy   = mlines.Line2D([], [], color="red",   linewidth=1.5, alpha=0.6, label=f"Dropped ({n_drop})")
    band_proxy  = mlines.Line2D([], [], color="black", linestyle="--", linewidth=1.2, label="F_bid*, F_ask*")
